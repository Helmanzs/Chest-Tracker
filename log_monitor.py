...
-----
    monitor = LogMonitor(
        log_path="game.log",
        chest_types=CHEST_DATA_SHEETS,        # {log_text: sheet_name}
        selected_chest=app.selected_chest_type,
        on_chest_detected=handle_chest,
        on_loot_item=handle_item,
        on_log=log_to_ui,
...
from constants import IGNORED_ITEMS, LOOT_TIMEOUT
...
class LogMonitor:
    ...
    def captured_loot(self) -> list[tuple[int, str]]:
        """Snapshot of loot collected so far (read-only use)."""
    ...
    def start_new_chest(self, chest_name: str) -> None:
        """
        Reset parse state for a freshly opened chest.
        Call this both from the log-tail path AND from the manual trigger.
        """
    ...
    def finalize(self) -> list[tuple[int, str]] | None:
        """
        Mark the current loot batch as complete and return collected items.
        Returns None if nothing was being collected.
        Guards against double-calls with an atomic flag swap.
        """
    ...
    def reset(self) -> None:
        """Discard any in-progress collection without saving."""
        self._awaiting_loot = False
        self._target_timestamp = None
        self._captured_loot = []
        self._last_loot_time = None
    ...
    def _timeout_monitor(self) -> None:
        """Background thread: fire on_timeout when loot collection goes quiet."""
        while self._running:
            if (
                self._awaiting_loot
                ...
                elapsed = time.time() - self._last_loot_time
                if elapsed >= self.loot_timeout:
    ...
    def _process_line(self, line: str) -> None:
        ...
        # Lock onto the timestamp of the first item in this batch
        ...
        if current_ts == self._target_timestamp:
            ...
            # A different timestamp means this loot belongs to a different event
